sap.ui.define([
  ...
  return BaseController.extend("com.bmore.inveweb.controller.vBeta", {
    onInit: function () {
      ...
      this.getRouter().getRoute("vBeta").attachPatternMatched(this._onContryMatched, this);
    },
    _onContryMatched: function (oEvent) {
      var sContryId = oEvent.getParameter("arguments").countyId;
      this.countryRegion = sContryId;
      this.getRankingbyDRB();
    },
    getRankingbyDRB: async function () {
      ...
      this.byId("title").setText("DVRs " + country + " DEL " + initTitle.toLocaleDateString("es-MX", options).toUpperCase() + " AL " + lastTitle.toLocaleDateString("es-MX", options).toUpperCase());
      try {
        let request = {
          "iniDate": iniDate,
          "endDate": endDate,
        ...
        let json = await this.execService(InveServices.GET_SUMARY_BY_DRV, request, "getRankingbyDRB", this.showLog);
    ...
    _onSearch: function (oEvent) {
      ...
      let oFilter = new Filter({
        filters: [
          new Filter("drvDesc", FilterOperator.Contains, sQuery)
        ],
        and: false
      });
    ...
    navContry: function (oEvent) {
      ...
      data.forEach(element => {
        if (element.drvDesc == oEvent.getSource().getTitle()) {
          idDrv = element.idDrv;
        }
      });
      this.navTo("vGamma", {
    ...
    download: async function () {
     ...
     switch (this.countryRegion) {
      ...
      XLSX.writeFile(wb, `${country} del ${iniDate} al ${endDate}.xlsx`);
    },
  });
});